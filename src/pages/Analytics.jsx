...
// Lightweight responsive hook — no library needed
...
export default function Analytics() {
  ...
  // Outer grid: template areas change per breakpoint
  const monthlyGridStyle = isMobile ? {
    display: 'grid',
    ...
    gridTemplateAreas: '"donut legend" "heatmap heatmap"',
    gap: '20px',
    alignItems: 'start',
  } : {
    // Desktop: all three in one row, heatmap pinned to its content width
    display: 'grid',
  ...
  // Legend inner grid columns
  const legendGridStyle = {
    gridArea: 'legend',
    display: 'grid',
    gridTemplateColumns: isMobile
      ? '1fr'                                   // 1 col — no overflow
      : isTablet
        ? 'repeat(2, 1fr)'                      // 2 cols on tablet
        : 'repeat(auto-fill, minmax(155px, 1fr))', // auto-fills on desktop
    gap: '10px 20px',
    alignContent: 'start',
  };

  // Category name — wrap on mobile, ellipsis on larger screens
  ...
  // ── Heatmap block (reused in both empty + filled states) ──────────────────
  const HeatmapBlock = () => (
    <div style={{ gridArea: 'heatmap' }}>
      ...
      {/* Scrollable wrapper prevents heatmap from overflowing on small screens */}
      ...
      <div style={s.card}>
        ...
        {pieData.length === 0 ? (
          ...
          <div style={monthlyGridStyle}>
            ...
            {/* 2 — Legend (dynamic columns via auto-fill) */}
                ...
                return (
                  <div key={i} style={s.legendItem}>
                    ...
                    <div style={s.legendDetails}>
                      <div style={legendNameStyle}>{item.icon} {item.name}</div>
...
const s = {
  ...
  // legendGridStyle and legendNameStyle are computed inline (breakpoint-aware)
  ...
  legendName:    { fontSize:'13px', color:'#fff', fontWeight:'500', marginBottom:'2px', overflow:'hidden', textOverflow:'ellipsis' },
  ...
  heatCell:      { width:'28px', height:'28px', borderRadius:'5px', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', transition:'transform 0.1s' },
